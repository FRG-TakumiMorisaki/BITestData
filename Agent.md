...
- 極端にきれいすぎるデータ（例: きっちり均等な分布、毎月全く同じ件数 など）は避け、
  ...
  - 例: 東日本のチームなら首都圏・東北の都市と製造/流通系の社名、公共・金融なら大都市圏の行政/金融調達先、
    グローバルなら英語圏・アジア圏の法人名と海外都市を組み合わせる。
  - 単なる通し番号や無機質なラベルは避け、営業担当・Department の設定と整合するようにする。
  - **CASE 文や単純な組み合わせ生成で一括命名しないこと。** 部門・担当者ごとに地域/業種の文脈を踏まえた固有名を用意し、担当が違えば会社名が被らないようにする。
